...
# ╔═╡ 3b3d724e-d3ea-11ed-2ad8-bf9ff7cb148a
begin
    using Plots
    using ControlSystemsBase
end

# ╔═╡ 3e1da2d3-77dd-44e5-bf50-885d41462933
md"""
# Polos e Zeros (LTI)
"""

# ╔═╡ bb5264d9-a4f5-4cfb-8e77-c2205ba6cef0
@enum WaveType begin
	sine
	square
	pulse
...
# ╔═╡ 733fed2f-95cb-487b-a038-be93c7317d0e
"""
```
function gensig(type::WaveType, tau::Int64)::Function
```
type: sine, square or pulse \n
tau: period
"""
function gensig(type::WaveType, tau::Int64)::Function
    type == sine   && return (x, t) ->  sin(t * (2π / tau))
    type == square && return (x, t) -> -sin(t * (2π / tau)) > 0 ? 1 : 0
    type == pulse  && return (x, t) -> (t % tau) == 0 ? 1 : 0
end;

# ╔═╡ 7fe9d274-82dd-46e1-89dc-a7849c56ce48
begin
    z = tf('z', 1)
    H = (0.05(z - 1)) / (z^2 - 1.85z + 0.9)
    u = gensig(square::WaveType, 20)
    res = lsim(H, u, 120)
end;

# ╔═╡ f7c3be68-bb49-4d9f-85ec-e8df4a59da79
H

# ╔═╡ 2012b555-6243-44ec-9267-a37326b1524d
plot(res.t, [res.u' 5res.y'];
	lines=:stem,
	xticks=0:10:120
)

# ╔═╡ fad162c7-54b9-40e7-b70e-b298d344b528
plot(pzmap(H))

# ╔═╡ 181d8c29-067c-4c46-b4de-290f9a83a3c1
begin
	H2 = (2/(1-0.9*(exp((1im* π)/4))*(z^-1)))+ (2/(1-0.9*(exp((-1im* π)/4))*(z^-1)))+(3/(1+2*(z^-1)))
	u2 = gensig(square::WaveType, 20)
	res2 = lsim(H2, u2, 101)
end;

# ╔═╡ 020c038b-5b87-4240-bfae-33a931fb918b
plot(res2.t, [res2.u' res2.y'];
	lines=:stem,
)

# ╔═╡ c57b94b2-febe-4133-9e47-2867067d9096
plot(pzmap(H2))
...
manifest_format = "2.0"
project_hash = "98afdefc2cdbfde56a0a47a3616b04cb3e323f90"
